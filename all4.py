...
def get_autosys_job_details(job_name):
    try:
        ...
        if match:
            ...
            print(f"Parsed Job Name: {job_name}, Start: {start_date}, End: {end_date}, Status: {status}")  # Debugging line
            ...
            # Return job details
            return status, start_timestamp, end_timestamp
        else:
...
def main(pattern, push_gateway_url):
    ...
    # Add job details to the gauge
    for job_name in job_names:
        status, start_timestamp, end_timestamp = get_autosys_job_details(job_name)
