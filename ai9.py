...
# Generate AI-Based Questions (Aligned with Static Scores)
def generate_questions(application: str, scores: list[int]):
    """
    Uses past ServiceNow tickets + static answers to generate AI questions.
    """
    relevant_tickets = ticket_data[ticket_data["Application"] == application]
    selected_tickets = relevant_tickets.sample(min(3, len(relevant_tickets)))  # Pick 3 tickets
...
def verify_skill(request: SkillVerificationRequest):
    ...
    return {"static_questions": STATIC_QUESTIONS, "ai_questions": ai_questions}
