...
def process_incident_data(rows):
    ...
    for row in rows:
        if len(row) != 5:
            logger.warning(f"Skipping row with unexpected column count: {row}")
            continue
...
def main():
    ...
    parser.add_argument('--jobname', required=True, help="Job name for Push Gateway")
    args = parser.parse_args()

    # Set server and database based on environment
    server = 'prods' if args.env == 'prod' else 'qas' if args.env == 'qa' else 'uas'
    ...
    if connection:
        ...
        push_metrics_to_gateway(incident_counts, args.pushgateway, jobname=args.jobname)
