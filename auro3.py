...
def push_job_statuses_to_pushgateway(job_statuses, gateway_url, grafana_job_name):
    ...
    delete_from_gateway(gateway_url, job=grafana_job_name)
    ...
    # Push all metrics to the Push Gateway with the dynamic Grafana job name
    push_to_gateway(gateway_url, job=grafana_job_name, registry=registry)

# Main function with argument parsing
def main():
    # Argument parser to handle env, push gateway URL, and Grafana job name
    ...
    parser.add_argument('--grafana-job-name', required=True, help="Grafana job name to display")
    ...
    # Push job statuses to the Push Gateway with the dynamic Grafana job name
    if job_statuses:
        push_job_statuses_to_pushgateway(job_statuses, args.gateway_url, args.grafana_job_name)
        print(f"Job statuses (on ice and on hold) pushed successfully with job name: {args.grafana_job_name}!")
    else:
