...
def check_urls_and_send_email(csv_file):
        ...
        for row in csv_reader:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    result = f"{url} is down with status code: {response.status_code}"
    ...
    # Constructing HTML table from CSV data with highlighting
    table_content = "<table><tr><th>URL</th><th>Status</th></tr>"
    for url, status in results:
        if "is up" in status:
            table_content += f"<tr class='up'><td>{url}</td><td>{status}</td></tr>"
        else:
            table_content += f"<tr class='down'><td>{url}</td><td>{status}</td></tr>"
    table_content += "</table>"
