...
with open(sql_output_file, "w", encoding="utf-8") as f:  # write SQL file
    for file_name in xls_files:
        ...
        try:
            ...
            # Find columns containing 'crds' AND ('code' OR 'codes')
            code_columns = [
                col for col in df.columns 
                if "crds" in col and ("code" in col or "codes" in col)
            ]
            ...
            if not codes:
                logging.warning(f"⚠️ No Crds code(s) found in file {file_name}. Skipping.")
                continue
