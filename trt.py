...
with open(sql_output_file, "w", encoding="utf-8") as f:
    for file_name in xls_files:
        ...
        try:
            # Read HTML disguised Excel
            with open(file_path, "r", encoding="latin1") as f_in:
                df_list = pd.read_html(f_in)
            df = df_list[0]

            # Normalize columns: lowercase + remove spaces
            norm_cols = [str(c).lower().replace(" ", "") for c in df.columns]
            df.columns = norm_cols

            # Find any column containing both "crds" and "code"
            code_columns = [col for col in df.columns if "crds" in col and "code" in col]

            if not code_columns:
                logging.warning(f"⚠️ No Crds code(s) column found in {file_name}. Skipping.")
                continue
            ...
            if not codes:
                logging.warning(f"⚠️ No code values found in {file_name}. Skipping.")
                continue
