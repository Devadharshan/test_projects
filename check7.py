...
def get_autosys_job_details(job_name):
    try:
        ...
        # Regex to match the actual job details line
        job_details_pattern = re.compile(
            r'^\s*(\S+)\s+(\d{2}/\d{2}/\d{2} \d{2}:\d{2}:\d{2})\s+(\d{2}/\d{2}/\d{2} \d{2}:\d{2}:\d{2})\s+(\S+)\s+(\S+)\s+(\S+)', 
            re.MULTILINE
        )
        ...
        if matches:
            for match in matches:
                ...
                print(f"Parsed Job Name: {job_name}, Status: {status}, Start: {start_date}, End: {end_date}, Run/Ntry: {run_ntry}, Pri/Xit: {pri_xit}")
...
def main(pattern, push_gateway_url):
    ...
    for job_name in job_names:
        ...
        if status == 'SU':
            ...
            print(f"Unknown status for job {job_name}: {status}")
