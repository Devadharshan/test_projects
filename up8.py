...
def load_jobsched_module():
    ...
    try:
        subprocess.run(["module", "load", "jobsched/QNA"], shell=True, check=True)
...
def run_autorep(job_patterns, history=False):
    """Runs autorep command using subprocess."""
    command = ["autorep", "-J", ",".join(job_patterns)]
    if history:
        command.append("-r")
        command.append("90")
